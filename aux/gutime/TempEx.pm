...
# Copyright 2004 - Georgetown University 
...
require      Exporter;
@ISA       = qw(Exporter);
@EXPORT    = qw($TE_Loaded $TE_EuroDate $TE_DEBUG $TE_HeurLevel
		TE_TagTIMEX TE_AddAttributes Date2ISO
		SetLexTagName SetEuroDate printNonConsumingTIMEXes);
...
use vars qw($TE_Loaded $TE_DEBUG $TE_EuroDate $TE_HeurLevel);
...
########################################
# Allows user to set toggle tag style between US and Euro
# US-Style = MM/DD/YYYY, Euro-Style = DD/MM/YYYY

sub SetEuroDate{
	my($eurosetting) = shift;
	$TE_EuroDate = $eurosetting;	
}
...
sub TE_TagTIMEX (\$) {
    ...
    #capture the decades  
    $rest = $string;
    while($rest =~/($OT+the$CT+\s*)(<TIMEX$tever[^>]*>)(\s*$OT+(\d\d\d\d)s$CT+)/giso)  {
	$rest = $5 . $';
	$b4 = $`;
	$mid1 = $4;	
	$mid1 .= "$1";	
...
#    my $numString = '(a? (few|couple))?$OT\+ (\d+|NUM_START.*?NUM_END)';  #will match either a numeric or word/based number
    ...
    #i.e. "3-year","four-month old"
       $matchPatterns[0] = "($OT\+$numString(-|$CT\+\\s$OT\+)$TE_Units((-|(s)?$CT\+\\s$OT\+)old)?$CT\+)";
...
#       $matchPatterns[4] = "($OT\+the$CT\+\\s$OT\+(NUM_ORD_STARTfirstNUM_ORD_END|initial|last|final)$CT\+\\s($OT\+$numString$CT\+)\\s$OT\+$TE_Units(s)?$CT\+\\s$OT\+of$CT\+)"; 
   ...
   #i.e. "a decade", "a few decades", NOT "a few hundred decades"
    ...
    $matchPatterns[12] = "(($OT\+(recent|several)$CT\+\\s)$OT\+$TE_Units(s)?$CT\+)";
...
EACHPAT: while ($string =~ /$curPattern/g){
    ...
    if($string =~ /season/io){
   	$string =~ s/($OT+(this|the|last|next|a|regular|last|sports|four)$CT+\s+$OT+(season)$CT)/<TIMEX$tever TYPE=\"DATE\">$1<\/TIMEX$tever>/sogi; 
    }
	...
	$string =~ s/($OT+the+$CT+\s+$OT+($TENumOrds|$TEOrdinalWords)$CT+\s+$OT+quarter$CT+)/<TIMEX$tever TYPE=\"DATE\">$1<\/TIMEX$tever>/sogi; 
	$string =~ s/($OT+the+$CT+\s+$OT+($TENumOrds|$TEOrdinalWords)-quarter$CT+)/<TIMEX$tever TYPE=\"DATE\">$1<\/TIMEX$tever>/sogi;
    }
...
## End of jfrank additions ##
    ...
    $string =~ s/(($OT+right$CT+\s+)?$OT+now$CT+)/<TIMEX$tever TYPE=\"DATE\">$1<\/TIMEX$tever>/sogi; 
    $string =~ s/($OT+recently$CT+)/<TIMEX$tever TYPE=\"DATE\">$1<\/TIMEX$tever>/sogi; 
    ...
    # This loop merges TIMEX of TIMEX ---> single timex
    $rest   = $string;
    $string = "";
    ...
    #set of rules for expanding left context of a tag 
    # This loop merges date followed by (this|next|last) year
    $rest   = $string;
    $string = "";
    while($rest =~/($OT(this|about|nearly|early|later|earlier|late)$CT\s*)(<TIMEX$tever[^>]*>)/giso)  {
	$rest = $';
	$b4 = $`;
	$mid1 = $5;	
	$mid1 .= "$3 ";	
    ...
    #set of rules for expanding right context of a tag 
    $rest   = $string;
    $string = "";
    while($rest =~/(<\/TIMEX$tever>)(\s*$OT+(later|earlier|late)$CT+\s*)/giso)  {
	$rest = $';
	$b4 = $`;
	$string = $b4 . $2 . $1 . $rest;	
   	$rest = $string;	
    ...
    #jbp: always expand left context for two word pairs 
    my @wpair = ("more than","up to","less than");
    my $w1;
    my $w2; 
    foreach (@wpair){  
    ...
    # general rule for expanding right context PP 
    # This loop expands right context for <TIMEX3></> PREP ....NNP 
    $rest   = $string;
    $string = "";
    my $NNtag = "(<[^\/][^>]*pos=NN[PS]?[^>]*>)";
    while($rest =~/((<\/TIMEX$tever>)([\s\n]*($OT+of|before|after|til|from|until|to$CT+)[\s\n]*($NNtag[^>]*>+\s*)+))/giso)  {
	$rest = $';
	$b4 = $`;
	$string = $3;
	$string .= $2;
...
sub TE_AddAttributes {
	...
	if($TEstring =~ /\b(late|end)\b/io) {
	    $Attributes .= " MOD=\"LATE\"";
	}elsif($TEstring =~ /\bno\s+more\s+than\b/io || $TEstring =~ /\bup\s+to\b/io){
...
###
	...
	while ($word =~ /([a-zA-Z]+)/g){
		...
		if (exists $word2Num{$curPart}){
			...
			print STDERR "Bad number put into wordToNumber.  Word is: $curPart\n";
			return 0;
		}
...
sub wordToNumberRecurse{
	...
	if (($numParts > 2) or ($numParts < 1)){
		# mihai
		printf STDERR "Found strange number string: \"%s\"\n", $numString;
		return 0;
		#die "error with number string in wordToNumberRecurse for above text\n";
	}
...
sub deliminateNumbers{
	...
	while ($rest =~ /$OT+([a-zA-Z-]+)$CT+/g){
		...
		} elsif($curWord =~ /^($numberTerm)(-$TE_Units(s)?)$/i){
				$addTerm = "$oTags$numStart$1$numEnd$3$cTags";		
				$middleOfNum =0;		
...
#####
	...
	if ($phrase =~ /^(\d*)[-\s]$TE_Units(s)?([-\s]old)?$/){
		$curVal = $1;
		$curUnit = $2;
...
#	} elsif ($phrase =~ /^the\s(1|last|final)\s(\d+)\s$TE_Units(s)?\sof$/){
	...
	#i.e. "a decade", "a couple years"
        } elsif ($phrase =~ /^a\s((few|several|couple|couple\sof)\s)?($TE_Units(s)?)$/){
		$curVal = $1;
		$curUnit = $3;
		if (defined($curVal)){
        ...
        } elsif ($phrase =~ /^((recent|few|several|couple)\s)($TE_Units(s)?)$/){
		$curVal = $1;
		$curUnit = $3;
		if (defined($curVal)){
			$curVal = "X"; 	#for underspecified values
...
=head1 Module TempEx
 ...
 Title:          GUTime(Formerly Temporal Expressions) 
 Version:        2.00
 Purpose:        Tagging temporal expressions
 Original Author:         Dr. George Wilson
 Organization:   The MITRE Corporation
 email:          gwilson@mitre.org
 Development Contact: Jon Phillips 
 Organization:   Georgetown University 
 email:          jbp24@georgetown.edu 
Date:		 Nov-2004

...
A summary of the temporal type system is provided in \refsec{types-review};
  ...
  key modifications from previous work are outlined in \refsec{differences};
...
Temporal expressions are represented either as a \ty{Range}, \ty{Sequence},
	or \ty{Duration}.
In addition, phrases can be tagged as a \ty{Function}, or as a \ty{Nil} type
...
This includes entities such as \te{Friday}, \te{November \th{27}}, or
	\te{last Friday}.
%(anatomy)
A \ty{Sequence} is defined as a partial completion of calendar fields.
For example, \te{November \th{27}} would define the month to be November
  and the day to be the \th{27}, but leave the year, hour, minute, etc.
  unspecified.
...
When considering that elements in the beam of possible groundings can become
  ...
  \tp{one hundred \th{5} second of Mondays from now}

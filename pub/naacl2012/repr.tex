...
A temporal representation is constructed; the type system of the representation
	...
	\refsec{repr-compositional} and descibed in detail in \refsec{repr-lexicon}
	and \refsec{repr-rules}
...
That is, \te{Today} spans a day, \te{1987} spans a year; \te{Now} has no span.
...
The sequence index $i \in \BZ$ is defined relative to the 
...
\end{equation}
\Fig{fig/gauss}{0.62}{distribution}{
...
For example, the distance between a Wednesday to that week's \te{Friday}
	would be $2/7$, counted with respect to a \te{Week}; 
	the distance from April to that year's \te{November} would be a 
	comparable $7/12$ (approximately), counted with respect to a \te{Month}.
...
Unlike many aspects of language  -- which are often not compositional or
	idomatic -- each word attached to a temporal phrase is 
...
% -- Synchronous Grammar
\FigStar{fig/grammar}{0.45}{grammar}{
...
This is in contrast to common approaches, such as making use of CCG 
	grammars (\newcite{key:1996steedman-ccg}, \newcite{key:2000steedman-ccg}),
...
The details are described in more detail in \refsec{repr-lexicon}.
...
\begin{itemize}
	...
	\item Applying (or partially applying) a function with one of its
		...
		\todo{reference logical paradigm}
	%(intersection)
